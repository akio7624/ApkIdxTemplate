...
|    unknown 2     |   -    | 4  | 알 수 없는 영역.<br>여러 파일에서 `02 00 00 00`이 공통으로 보인다.                  |
...
이 부분도 중요하다.
아카이브 자체도 패딩이 필요하다.
아카이브의 패딩 방식은 ROOT 파일의 패딩 방식과 같다.

위에서 설명한 $(512 \times n) - 1$에 따라서 패딩을 하면 된다.
이것이 바로 위에서 아카이브가 일종의 ROOT 파일이라고 말한 이유이다.
